...
describe('calculateVisualProgress', () => {
  it('returns start position at progress 0', () => {
    expect(calculateVisualProgress(0, 0, 0)).toBe(0.1);
  });

  it('keeps boats in the center (50%) during the middle phase of the race', () => {
    expect(calculateVisualProgress(0.5, 0.5, 0.5)).toBe(0.5);
  });

  it('spreads leading and trailing boats around the visual center', () => {
    const leading = calculateVisualProgress(0.6, 0.5, 0.5);
    expect(leading).toBeCloseTo(0.63);

    const trailing = calculateVisualProgress(0.4, 0.5, 0.5);
    expect(trailing).toBeCloseTo(0.37);
  });

  it('sprints towards the finish line (90%) in the final 15% of the race', () => {
    expect(calculateVisualProgress(1.0, 1.0, 1.0)).toBeCloseTo(0.9);
  });

  it('keeps visual progress bounded safely', () => {
    expect(calculateVisualProgress(1.5, 0.5, 0.5)).toBe(0.98);
    expect(calculateVisualProgress(-0.5, 0.5, 0.5)).toBe(0.02);
  });
});
...
describe('buildRaceRedirectUrl', () => {
  ...
  it('appends collection params when not default', () => {
    const url = buildRaceRedirectUrl(
      'abc123',
      ['Alice'],
      30,
      'flags-of-us',
...
describe('formatDisplayName', () => {
  ...
  it('slices long names and appends ellipses', () => {
    expect(formatDisplayName('Alice Thompson Rivera Morales', 15, 10)).toBe(
      'Alice Thom...'
    );
  });
...
describe('updateWafflePosition', () => {
  it('skips update if waffle is already finished', () => {
    ...
    const res = updateWafflePosition(
      waffle,
      0.016,
      0.5,
      0.5,
